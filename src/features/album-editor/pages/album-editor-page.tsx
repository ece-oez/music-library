...
function AlbumEditorForm({ albumId, initialAlbum }: AlbumEditorFormProps) {
  ...
  const importPlaylist = useMutation({
    mutationFn: (url: string) => repositories.music.importYouTubePlaylist(url),
    onSuccess: (importedAlbum) => {
      setTitle(importedAlbum.title)
      setArtist(importedAlbum.artist)
      setReleaseYear(importedAlbum.releaseYear > 0 ? String(importedAlbum.releaseYear) : '')
      setGenre(importedAlbum.genre)
      setArtworkUrl(importedAlbum.artworkUrl ?? '')
      setTracks(importedAlbum.tracks.map((track) => ({ id: createId('track'), title: track.title, duration: track.duration, youtubeUrl: track.youtubeUrl })))
    },
  })
  ...
  return (
    <div className="editor-page album-editor-page">
      ...
      <form className="editor-form" onSubmit={handleSubmit}>
        {!isEditing && <fieldset className="playlist-import"><legend>Import from YouTube</legend><p className="field-help">Paste a public YouTube playlist URL. The playlist title, channel, artwork, tracks, durations and links will be imported.</p><div className="import-row"><input aria-label="YouTube playlist URL" type="url" value={playlistUrl} onChange={(event) => setPlaylistUrl(event.target.value)} placeholder="https://www.youtube.com/playlist?list=..." /><button className="secondary-button" disabled={importPlaylist.isPending || !playlistUrl.trim()} onClick={() => importPlaylist.mutate(playlistUrl.trim())} type="button">{importPlaylist.isPending ? 'Importing...' : 'Import playlist'}</button></div>{importPlaylist.isError && <p className="form-error">{importPlaylist.error instanceof Error ? importPlaylist.error.message : 'The YouTube playlist could not be imported.'}</p>}</fieldset>}
        ...
        <div className="form-row"><label>Release year<input min="1900" max="2100" type="number" value={releaseYear} onChange={(event) => setReleaseYear(event.target.value)} placeholder="Unknown year" /></label><label>Genre<input required value={genre} onChange={(event) => setGenre(event.target.value)} placeholder="Jazz" /></label></div>
